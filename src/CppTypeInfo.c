...
void print_Function(const Function *func) {
  ...
  for (size_t i = 0; i < func->funcCount; i++) {
    ...
    printf(
        "      %s %s(",
        CXX_Type_TO_char_p(func_type.returnTypeCXX, func_type.returnTypeInfo),
    ...
    for (size_t j = 0; j < func_type.argsCount; j++) {
      printf("%s, ",
             CXX_Type_TO_char_p(CXX_Type_array_getat(func_type.argsTypeCXX, j),
...
void print_Symbols(const Symbols *symbols) {
  ...
  printf("\n  Functions: \n");
  DS_LIST_PTR_FOREACH(symbols->funcs, func) {
    print_Function(func);
  }

  printf("\n  Globals: \n");
  DS_LIST_PTR_FOREACH(symbols->globals, global) {
    print_Global(global);
  }

  printf("\n  Structures: \n");
  DS_LIST_PTR_FOREACH(symbols->structs, s) {
    print_Structure(s);
  }

  printf("\n  Union: \n");
  DS_LIST_PTR_FOREACH(symbols->unions, u) {
    print_Union(u);
  }

  printf("\n  TypeDefs: \n");
  DS_LIST_PTR_FOREACH(symbols->typedefs, td) {
    print_TypeDef(td);
  }

  printf("\n  Enums: \n");
  DS_LIST_PTR_FOREACH(symbols->enums, e) {
    print_Enum(e);
  }
}
...
void *get_underlyingTypeInfo(CXType cxType,
                             const Symbols *sym,
  ...
  if (cxType.kind == CXType_Typedef) {
    ...
    return get_underlyingTypeInfo(underlying_type,
                                  sym,
                                  cxx_type,
                                  objects_ffi_type);
...
enum CXChildVisitResult
record_visitor(CXCursor cursor, CXCursor parent, CXClientData client_data) {
...
FunctionType Function_parser(CXCursor funcCursor, const Symbols *symbols) {
  ...
  void *extra_return_type_info = get_underlyingTypeInfo(
      clang_getResultType(clang_getCursorType(funcCursor)),
      symbols,
      &cxx_return_type,
      &ffi_return_type);
  ...
  for (int i = 0; i < args_len; i++) {
    ...
    void *extra_arg_type_info =
        get_underlyingTypeInfo(arg_type, symbols, &cxx_arg_type, &ffi_arg_type);
...
visitor(CXCursor cursor, CXCursor parent, CXClientData client_data) {
  ...
  // typedef
    ...
    void *underlying_type_info = get_underlyingTypeInfo(underlying_type,
                                                        symbols,
                                                        &td_cxx_type,
                                                        &td_ffi_type);

    if (!Symbols_appendTypedef(symbols,
                               typedef_name,
                               type_name,
                               td_cxx_type,
  ...
  // global variables
    ...
    void *underlying_type_info = get_underlyingTypeInfo(type,
                                                        symbols,
                                                        &(global.typeCXX),
                                                        &(global.typeFFI));

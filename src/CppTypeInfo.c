...
void print_Function(Function func)
{
    ...
    for (size_t i = 0; i < func.funcCount; i++)
    {
        ...
        FunctionType type = *((FunctionType*)qvector_getat(func.functionTypes, i, false));
...
void print_Symbols(Symbols *symbols)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        ...
        Structure s = *((Structure*)qhashtbl_get(symbols->structs, name, NULL, false));
        print_Structure(s);
    }
...
bool Symbols_appendFunction(Symbols *sym, const char *name, const char *mangledName, FunctionType funcType)
{
    ...
    else
    {
        ...
        q.functionTypes = qvector(MAX_SIZE, sizeof(FunctionType), QVECTOR_RESIZE_EXACT);
        if (!(q.functionTypes))
            goto raise_error;

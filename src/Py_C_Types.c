        ...
        if (PySequence_Check(arg_1)) {                                                                                 \
            ...
            for (size_t i = 0; i < len; i++) {                                                                         \
                ...
                if (type_check_func(element)) {                                                                        \
                    ...
                    PyErr_SetString(PyExc_TypeError, "Expected all elements of the sequence to be of type " #name);    \
                    return -1;                                                                                         \
                }                                                                                                      \
            }                                                                                                          \
            return 0;                                                                                                  \
...
NEW_PY_CTYPE_DEL(c_char, char, &py_c_type_type, CPyChar_Check, PyLong_AsLong, PyLong_FromLong, PyLong_FromLong);
...
static PyObject *c_struct_append(PyObject *self, PyObject *args) {
    ...
    void *data = selfType->pointer + (selfType->structure->structSize * selfType->arraySize);
...
static PyObject *c_struct_pop(PyObject *self) {
    ...
    // pop
    void     *data = selfType->pointer + (selfType->structure->structSize * selfType->arraySize - 1);
    ...
    if ((selfType->arraySize * 2 + 1) < selfType->arrayCapacity) {
        selfType->pointer = realloc(selfType->pointer, (selfType->arraySize + 1) * selfType->structure->structSize);
...
static PyObject *c_union_next(PyObject *self) {
    ...
    if (selfType->arraySize > index) {
        ...
        return cppArg_to_pyArg(data, CXX_Struct, selfType->u, selfType->parentModule);
    }
    (selfType->_i)++;
...
static PyObject *c_union_pop(PyObject *self) {
    ...
    // BUG: I guess this is flawed logic to pop the memory should be copied before
    // pop
    void     *data = selfType->pointer + (selfType->u->unionSize * selfType->arraySize - 1);
    PyObject *tmp  = cppArg_to_pyArg(data, CXX_Union, selfType->u, selfType->parentModule);

    (selfType->arraySize)--;

    if ((selfType->arraySize * 2 + 1) < selfType->arrayCapacity) {
        selfType->pointer       = realloc(selfType->pointer, (selfType->arraySize + 1) * selfType->u->unionSize);
        selfType->arrayCapacity = selfType->arraySize * 2 + 1;
    }

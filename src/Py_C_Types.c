...
#define NEW_PY_CTYPE_DEL(                                                            \
    name, type, base, type_check_func, type_convert_pyobj_to_ctype,                  \
    type_convert_ctype_to_pyobj, type_convert_ctype_to_pyint)                        \
    ...
    PyMemberDef name##_members[] = {                                                 \
        {"is_pointer", T_BOOL, offsetof(PyC_##name, isPointer), READONLY,            \
         "PyC." #name ".is_pointer"},                                                \
        {"is_array", T_BOOL, offsetof(PyC_##name, isArray), READONLY,                \
         "PyC." #name ".is_array"},                                                  \
        {NULL, 0, 0, 0, NULL},                                                       \
    };                                                                               \
    PyGetSetDef name##_getsetdef[] = {                                               \
        {"free_on_no_reference", name##_freeOnDel_getter,                            \
         name##_freeOnDel_setter, #name ".free_on_no_reference", NULL},              \
        {NULL, NULL, NULL, NULL},                                                    \
    };                                                                               \
                                                                                     \
        ...
        if (PySequence_Check(arg_1)) {                                               \
            ...
            for (size_t i = 0; i < len; i++) {                                       \
                ...
                if (type_check_func(element)) {                                      \
                    selfType->pointer[i] =                                           \
                        type_convert_pyobj_to_ctype(element);                        \
                } else {                                                             \
                    PyErr_SetString(PyExc_TypeError,                                 \
                                    "Expected all elements of the sequence "         \
                                    "to be of type " #name);                         \
                    return -1;                                                       \
                }                                                                    \
            }                                                                        \
    ...
    PyObject *name##_iter(PyObject *self) {                                          \
        ...
        PyErr_SetString(py_CppError,                                                 \
                        "given " #name                                               \
                        " instance is not an array type instance");                  \
        return NULL;                                                                 \
    }                                                                                \
                                                                                     \
    /* __next__ */                                                                   \
    PyObject *name##_next(PyObject *self) {                                          \
        ...
        if (selfType->_i < selfType->arraySize)                                      \
            return type_convert_ctype_to_pyobj(                                      \
                selfType->pointer[(selfType->_i)++]);                                \
                                                                                     \
        return NULL;                                                                 \
    }                                                                                \
                                                                                     \
    ...
    PyObject *name##_freeOnDel_getter(PyObject *self, void *closure) {               \
        ...
        Py_RETURN_FALSE;                                                             \
                                                                                     ...
                                                                                     \
    /* freeOnDel setter */                                                           \
    int name##_freeOnDel_setter(PyObject *self, PyObject *value,                     \
                                void *closure) {                                     \
        if (!value) {                                                                \
    ...
    PyObject *name##_append(PyObject *self, PyObject *py_value) {                    \
        ...
        if (selfType->isArray == false) {                                            \
            PyErr_SetString(PyExc_TypeError, "given instance of " #name              \
    ...
    PyObject *name##_pop(PyObject *self) {                                           \
        ...
        if (selfType->arraySize == 0) {                                              \
            PyErr_SetString(PyExc_IndexError,                                        \
                            "no elements in the array to pop");                      \
            return NULL;                                                             \
        }                                                                            \
                                                                                     \
        ...
        if ((selfType->arraySize * 2) < selfType->arrayCapacity) {                   \
            selfType->pointer       = realloc(selfType->pointer,                     \
                                              (selfType->arraySize) * sizeof(type)); \
...
NEW_PY_CTYPE_DEL(c_char, char, &py_c_type_type, CPyChar_Check, CPyChar_AsChar,
                 CPyChar_FromChar, PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_uchar, unsigned char, &py_c_type_type, PyNumber_Check,
                 PyLong_AsUnsignedLong, PyLong_FromUnsignedLong,
                 PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_short, short, &py_c_type_type, PyNumber_Check,
                 longlong_from_PyNumber, PyLong_FromLong, PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_ushort, unsigned short, &py_c_type_type, PyNumber_Check,
                 ulonglong_from_PyNumber, PyLong_FromUnsignedLong,
                 PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_int, int, &py_c_type_type, PyNumber_Check,
                 longlong_from_PyNumber, PyLong_FromLong, PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_uint, unsigned int, &py_c_type_type, PyNumber_Check,
                 ulonglong_from_PyNumber, PyLong_FromUnsignedLong,
                 PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_long, long, &py_c_type_type, PyNumber_Check,
                 longlong_from_PyNumber, PyLong_FromLong, PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_ulong, unsigned long, &py_c_type_type, PyNumber_Check,
                 ulonglong_from_PyNumber, PyLong_FromUnsignedLong,
                 PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_float, float, &py_c_type_type, CPyFloat_Check,
                 double_from_PyNumber, PyFloat_FromDouble, PyLong_FromDouble);

NEW_PY_CTYPE_DEL(c_double, double, &py_c_type_type, CPyFloat_Check,
                 double_from_PyNumber, PyFloat_FromDouble, PyLong_FromDouble);

NEW_PY_CTYPE_DEL(c_bool, bool, &py_c_type_type, CPyBool_Check, PyObject_IsTrue,
                 PyBool_FromLong, PyLong_FromLong);
    ...
    for (size_t i = 0; i < selfType->structure->attrCount; i++) {
        if (strcmp(attr, str_array_getat(selfType->structure->attrNames, i)) ==
            ...
            return cppArg_to_pyArg(
                data + (long_long_array_getat(selfType->structure->offsets, i) /
                        8),
                CXX_Type_array_getat(selfType->structure->attrTypeCXX, i),
...
static int c_struct_setattr(PyObject *self, char *attr, PyObject *pValue) {
    ...
    for (size_t i = 0; i < selfType->structure->attrCount; i++) {
        if (strcmp(attr, str_array_getat(selfType->structure->attrNames, i)) ==
            ...
            memcpy((char *)(selfType->pointer) +
                       (long_long_array_getat(selfType->structure->offsets, i) /
                        8),
                   data, type->size);
...
PyObject *create_py_c_struct(Structure *structure, PyObject *module) {
    ...
    *py_c_new_type = (struct Custom_s_PyTypeObject){
    ...
    if (PyModule_AddObject(PyC, structure->name, (PyObject *)py_c_new_type) <
        0) {
...
static PyObject *c_struct_next(PyObject *self) {
    ...
    if (selfType->arraySize > index) {
        rvalue = cppArg_to_pyArg(
            selfType->pointer + (selfType->structure->structSize * index),
            CXX_Struct, selfType->structure, selfType->parentModule);
    }
...
static PyObject *c_struct_append(PyObject *self, PyObject *args) {
    ...
    if (selfType->arrayCapacity > selfType->arraySize) {
        ...
        selfType->pointer = realloc(
            selfType->pointer, new_capacity * selfType->structure->structSize);
        selfType->arrayCapacity = new_capacity;
    ...
    for (size_t i = 0; i < selfType->u->attrCount; i++) {
        if (!(strcmp(attr, str_array_getat(selfType->u->attrNames, i)))) {
            enum CXX_Type cxx_type =
                CXX_Type_array_getat(selfType->u->attrTypeCXX, i);
            ffi_type *type =
                p_ffi_type_array_getat(selfType->u->attrTypeFFI, i);
...
PyObject *create_py_c_union(Union *u, PyObject *module) {
    ...
    *py_c_new_type = (struct Custom_u_PyTypeObject){
    ...
    PyErr_SetString(py_BindingError,
                    "Unable to access PyCpp.c_union base class");
    return NULL;
}
...
static PyObject *c_union_append(PyObject *self, PyObject *args) {
    ...
    if (selfType->arrayCapacity > selfType->arraySize) {
        memcpy(selfType->pointer +
                   (selfType->u->unionSize * selfType->arraySize),

...
#define NEW_PY_CTYPE_DEL(name, type, base, type_check_func, type_convert_pyobj_to_ctype, type_convert_ctype_to_pyobj,  \
    ...
    PyMemberDef name##_members[] = {                                                                                   \
        {"is_pointer", T_BOOL, offsetof(PyC_##name, isPointer), READONLY, "PyC." #name ".is_pointer"},                 \
        {"is_array", T_BOOL, offsetof(PyC_##name, isArray), READONLY, "PyC." #name ".is_array"},                       \
        {NULL, 0, 0, 0, NULL},                                                                                         \
    };                                                                                                                 \
    PyGetSetDef name##_getsetdef[] = {                                                                                 \
        {"free_on_no_reference", name##_freeOnDel_getter, name##_freeOnDel_setter, #name ".free_on_no_reference",      \
         NULL},                                                                                                        \
        {NULL, NULL, NULL, NULL},                                                                                      \
    };                                                                                                                 \
                                                                                                                       \
    ...
    int name##_init(PyObject *self, PyObject *args, PyObject *kwargs) {                                                \
        ...
        if (!arg_1) {                                                                                                  \
            PyErr_SetString(PyExc_TypeError, "Expected " #type " or sequence of " #name);                              \
            return -1;                                                                                                 \
        }                                                                                                              \
                                                                                                                       \
        ...
        if (PySequence_Check(arg_1)) {                                                                                 \
            ...
            selfType->pointer       = calloc(len + 1, sizeof(type));                                                   \
            ...
            for (size_t i = 0; i < len; i++) {                                                                         \
                ...
                if (type_check_func(element)) {                                                                        \
                    ...
                    PyErr_SetString(PyExc_TypeError, "Expected all elements of the sequence "                          \
                                                     "to be of type " #name);                                          \
                    return -1;                                                                                         \
                }                                                                                                      \
            }                                                                                                          \
    ...
    PyObject *name##_iter(PyObject *self) {                                                                            \
        ...
        if (selfType->isArray) {                                                                                       \
            ...
            return self;                                                                                               \
                                                                                                                       ...
                                                                                                                       \
        PyErr_SetString(py_CppError, "given " #name " instance is not an array type instance");                        \
        return NULL;                                                                                                   \
    }                                                                                                                  \
                                                                                                                       \
    /* __next__ */                                                                                                     \
    ...
    int name##_freeOnDel_setter(PyObject *self, PyObject *value, void *closure) {                                      \
        if (!value) {                                                                                                  \
            PyErr_SetString(PyExc_AttributeError, "Cannot delete " #name ".free_on_no_reference attribute");           \
            return -1;                                                                                                 \
        }                                                                                                              \
                                                                                                                       \
    ...
    PyObject *name##_append(PyObject *self, PyObject *py_value) {                                                      \
        ...
        if (selfType->isArray == false) {                                                                              \
            PyErr_SetString(PyExc_TypeError, "given instance of " #name "is not an array type instance");              \
            return NULL;                                                                                               \
        }                                                                                                              \
                                                                                                                       \
        if (type_check_func(py_value)) {                                                                               \
            ...
            if (selfType->arrayCapacity > selfType->arraySize) {                                                       \
                ...
                int new_capacity        = selfType->arrayCapacity ? (selfType->arrayCapacity * 2) : 2;                 \
                selfType->pointer       = realloc(selfType->pointer, new_capacity * sizeof(type));                     \
        ...
        PyErr_SetString(PyExc_TypeError, "Expected integer type got some other type");                                 \
        return NULL;                                                                                                   \
    }                                                                                                                  \
                                                                                                                       \
    /* pop */                                                                                                          \
    PyObject *name##_pop(PyObject *self) {                                                                             \
        ...
        if (selfType->arraySize == 0) {                                                                                \
            ...
            return NULL;                                                                                               \
                                                                                                                       ...
                                                                                                                       \
        PyObject *rvalue = type_convert_ctype_to_pyobj(selfType->pointer[--(selfType->arraySize)]);                    \
                                                                                                                       \
        if ((selfType->arraySize * 2) < selfType->arrayCapacity) {                                                     \
            selfType->pointer       = realloc(selfType->pointer, (selfType->arraySize) * sizeof(type));                \
...
bool CPyChar_Check(PyObject *obj) {
    Py_ssize_t size = 0;
    if (PyNumber_Check(obj) || (PyUnicode_Check(obj) && PyUnicode_AsUTF8AndSize(obj, &size) && size <= 1)) {
        return 1;
    }
    return 0;
}
...
PyObject *c_char_str(PyObject *self) {
    ...
    if (selfType->isPointer) {
        return PyUnicode_FromFormat("%s", selfType->pointer);
    }
    char c[2] = {selfType->value, 0};
    return PyUnicode_FromFormat("%s", c);
}

NEW_PY_CTYPE_DEL(c_char, char, &py_c_type_type, CPyChar_Check, CPyChar_AsChar, CPyChar_FromChar, PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_uchar, unsigned char, &py_c_type_type, PyNumber_Check, PyLong_AsUnsignedLong,
                 PyLong_FromUnsignedLong, PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_short, short, &py_c_type_type, PyNumber_Check, longlong_from_PyNumber, PyLong_FromLong,
                 PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_ushort, unsigned short, &py_c_type_type, PyNumber_Check, ulonglong_from_PyNumber,
                 PyLong_FromUnsignedLong, PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_int, int, &py_c_type_type, PyNumber_Check, longlong_from_PyNumber, PyLong_FromLong, PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_uint, unsigned int, &py_c_type_type, PyNumber_Check, ulonglong_from_PyNumber,
                 PyLong_FromUnsignedLong, PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_long, long, &py_c_type_type, PyNumber_Check, longlong_from_PyNumber, PyLong_FromLong,
                 PyLong_FromLong);

NEW_PY_CTYPE_DEL(c_ulong, unsigned long, &py_c_type_type, PyNumber_Check, ulonglong_from_PyNumber,
                 PyLong_FromUnsignedLong, PyLong_FromUnsignedLong);

NEW_PY_CTYPE_DEL(c_float, float, &py_c_type_type, CPyFloat_Check, double_from_PyNumber, PyFloat_FromDouble,
                 PyLong_FromDouble);

NEW_PY_CTYPE_DEL(c_double, double, &py_c_type_type, CPyFloat_Check, double_from_PyNumber, PyFloat_FromDouble,
                 PyLong_FromDouble);

NEW_PY_CTYPE_DEL(c_bool, bool, &py_c_type_type, CPyBool_Check, PyObject_IsTrue, PyBool_FromLong, PyLong_FromLong);
...
PyGetSetDef c_void_getsetdef[] = {
    {"free_on_no_reference", &c_void_freeOnDel_getter, &c_void_freeOnDel_setter, C_TYPE_FREE_ON_NO_REFERENCE_DOC, NULL},
    {NULL, NULL, NULL, NULL},
};

PyTypeObject py_c_void_type = {
    PyVarObject_HEAD_INIT(NULL, 0) //
        .tp_name  = "PyCpp.c_void",
...
static int c_void_freeOnDel_setter(PyObject *self, PyObject *value, void *closure) {
    PyC_c_void *selfType = (PyC_c_void *)self;

    if (!value) {
        PyErr_SetString(PyExc_AttributeError, "Cannot delete c_void.free_on_no_reference attrubute");
        return -1;
    }
...
PyMemberDef c_struct_members[] = {
    {"is_array", T_BOOL, offsetof(PyC_c_struct, isArray), READONLY, "PyC.c_struct.is_array"},
    // {"__python_representation", T_OBJECT, offsetof(PyC_c_struct,
    // pyDictRepr),
    //  READONLY, "PyC.c_struct.__python_representation"},
    {NULL, 0, 0, 0, NULL}};

PyMethodDef c_struct_methods[] = {{"append", (PyCFunction)&c_struct_append, METH_VARARGS, "c_struct.append()"},
                                  {"pop", (PyCFunction)&c_struct_pop, METH_NOARGS, "c_struct.pop()"},
                                  {NULL, NULL, 0, NULL}};

PyGetSetDef c_struct_getsetdef[] = {
    {"free_on_no_reference", &c_struct_freeOnDel_getter, &c_struct_freeOnDel_setter, C_TYPE_FREE_ON_NO_REFERENCE_DOC,
     NULL},
    {NULL, NULL, NULL, NULL},
};
...
static int c_struct_init(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    selfType->parentModule = ((struct Custom_s_PyTypeObject *)self->ob_type)->parentModule;
    ...
    if (args_len == 1 && PySequence_Check(PyTuple_GetItem(args, 0))) {
        ...
        selfType->pointer       = realloc(selfType->pointer, s->structSize * (arr_len + 1));
        selfType->isArray       = true;
        selfType->isPointer     = true;
        ...
        for (size_t i = 0; i < arr_len; i++) {
            ...
            if (Py_TYPE(element) != Py_TYPE(element)) {
                PyErr_SetString(py_BindingError, "Expected all elements of the sequence to be of the "
                                                 "same struct type");
                return -1;
            }

            PyC_c_struct *elementType = (PyC_c_struct *)element;
            memcpy(selfType->pointer + (s->structSize * i), elementType->pointer, s->structSize);
    ...
    if (args_len != s->attrCount) {
        PyErr_SetString(py_BindingError, "Args count does not match struct's attribute count. "
                                         "Cannot initilise the struct.");
        return -1;
    }

    for (size_t i = 0; i < selfType->structure->attrCount; i++) {
        ffi_type *type         = p_ffi_type_array_getat(selfType->structure->attrTypeFFI, i);
        enum CXX_Type cxx_type = CXX_Type_array_getat(selfType->structure->attrTypeCXX, i);
        ...
        memcpy((char *)(selfType->pointer) + (long_long_array_getat(selfType->structure->offsets, i) / 8), data,
               type->size);
...
static PyObject *c_struct_getattr(PyObject *self, char *attr) {
    ...
    for (size_t i = 0; i < selfType->structure->attrCount; i++) {
        if (strcmp(attr, str_array_getat(selfType->structure->attrNames, i)) == 0) {
            ...
            return cppArg_to_pyArg(data + (long_long_array_getat(selfType->structure->offsets, i) / 8),
                                   CXX_Type_array_getat(selfType->structure->attrTypeCXX, i),
                                   p_void_array_getat(selfType->structure->attrTypeInfo, i), selfType->parentModule);
        }
    }
...
PyObject *create_py_c_struct(Structure *structure, PyObject *module) {
    ...
    char *struct_name = malloc(7 + strlen(structure->name)); // TODO: free this malloc
    ...
    *py_c_new_type = (struct Custom_s_PyTypeObject){
    ...
    PyErr_SetString(py_BindingError, "Unable to access PyCpp.c_struct base class");
    return NULL;
}
...
static PyObject *c_struct_iter(PyObject *self) {
    ...
    PyErr_SetString(py_CppError, "given c_struct instance is not an array type instance");
    return NULL;
}
...
static PyObject *c_struct_next(PyObject *self) {
    ...
    if (selfType->arraySize > index) {
        rvalue = cppArg_to_pyArg(selfType->pointer + (selfType->structure->structSize * index), CXX_Struct,
                                 selfType->structure, selfType->parentModule);
    }
...
static PyObject *c_struct_append(PyObject *self, PyObject *args) {
    ...
    if (!(selfType->isArray)) {
        PyErr_SetString(py_CppError, "given instance of c_struct is not an array type instance");
        return NULL;
    }
    ...
    if (Py_TYPE(value) != Py_TYPE(self)) {
        PyErr_SetString(py_BindingError, "Expected arg to be of the same struct type");
        return NULL;
    }
    ...
    if (selfType->arrayCapacity > selfType->arraySize) {
        memcpy(selfType->pointer + (selfType->structure->structSize * selfType->arraySize), valueType->pointer,
               selfType->structure->structSize);
    } else {
        int new_capacity        = selfType->arrayCapacity ? (selfType->arrayCapacity * 2) : 2;
        selfType->pointer       = realloc(selfType->pointer, new_capacity * selfType->structure->structSize);
        selfType->arrayCapacity = new_capacity;
...
static PyObject *c_struct_pop(PyObject *self) {
    ...
    memcpy(data, ((char *)selfType->pointer) + (selfType->structure->structSize * (selfType->arraySize - 1)),
           selfType->structure->structSize);

    PyObject *tmp = cppArg_to_pyArg(data, CXX_Struct, selfType->structure, selfType->parentModule);

    (selfType->arraySize)--;

    if ((selfType->arraySize * 2) < selfType->arrayCapacity) {
        selfType->pointer       = realloc(selfType->pointer, (selfType->arraySize) * selfType->structure->structSize);
        selfType->arrayCapacity = selfType->arraySize;
    }
...
static PyObject *c_struct_getitem(PyObject *self, PyObject *attr) {
    ...
    if (!(PyNumber_Check(attr))) {
        PyErr_SetString(PyExc_TypeError, "Expected interger type got some other type");
        return NULL;
    }
    ...
    if (selfType->arraySize > index) {
        ...
        return cppArg_to_pyArg(data, CXX_Struct, selfType->structure, selfType->parentModule);
    }
...
static int c_struct_setitem(PyObject *self, PyObject *attr, PyObject *value) {
    ...
    if (selfType->arraySize > index) {
        ...
        memcpy(selfType->pointer + (selfType->structure->structSize * index), valueType->pointer,
               selfType->structure->structSize);
...
static int c_struct_freeOnDel_setter(PyObject *self, PyObject *value, void *closure) {
    PyC_c_struct *selfType = (PyC_c_struct *)self;

    if (!value) {
        PyErr_SetString(PyExc_AttributeError, "Cannot delete c_struct.free_on_no_reference attrubute");
        return -1;
    }
...
PyMemberDef c_union_members[] = {{"is_array", T_BOOL, offsetof(PyC_c_union, isArray), READONLY, "PyC.c_union.is_array"},
                                 // {"__python_representation", T_OBJECT, offsetof(PyC_c_union,
                                 // pyDictRepr),
                                 //  READONLY, "PyC.c_union.__python_representation"},
                                 {NULL, 0, 0, 0, NULL}};

PyMethodDef c_union_methods[] = {{"append", (PyCFunction)&c_union_append, METH_VARARGS, "c_union.append()"},
                                 {"pop", (PyCFunction)&c_union_pop, METH_NOARGS, "c_union.pop()"},
                                 {NULL, NULL, 0, NULL}};

PyGetSetDef c_union_getsetdef[] = {
    {"free_on_no_reference", &c_union_freeOnDel_getter, &c_union_freeOnDel_setter, C_TYPE_FREE_ON_NO_REFERENCE_DOC,
     NULL},
    {NULL, NULL, NULL, NULL},
};
...
static int c_union_init(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if ((args_len == 1) && PySequence_Check(PyTuple_GetItem(args, 0))) {
        ...
        for (size_t i = 0; i < arr_len; i++) {
            ...
            PyC_c_struct *elementType = (PyC_c_struct *)element;
            memcpy(selfType->pointer + (s->unionSize * i), elementType->pointer, s->unionSize);
        }

        return 0;
    }

    PyErr_SetString(PyExc_TypeError, "Expected no arguments or sequence of c_union, but got something else");
...
static PyObject *c_union_getattr(PyObject *self, char *attr) {
    ...
    for (size_t i = 0; i < selfType->u->attrCount; i++) {
        if (!(strcmp(attr, str_array_getat(selfType->u->attrNames, i)))) {
            return cppArg_to_pyArg((selfType->pointer), CXX_Type_array_getat(selfType->u->attrTypeCXX, i),
                                   p_void_array_getat(selfType->u->attrTypeInfo, i), selfType->parentModule);
        }
    }
...
static int c_union_setattr(PyObject *self, char *attr, PyObject *pValue) {
    ...
    for (size_t i = 0; i < selfType->u->attrCount; i++) {
        if (!(strcmp(attr, str_array_getat(selfType->u->attrNames, i)))) {
            enum CXX_Type cxx_type = CXX_Type_array_getat(selfType->u->attrTypeCXX, i);
...
static PyObject *c_union_iter(PyObject *self) {
    ...
    PyErr_SetString(py_CppError, "given c_union instance is not an array type instance");
    return NULL;
}
...
static PyObject *c_union_next(PyObject *self) {
    ...
    if (selfType->arraySize > index) {
        ...
        rvalue     = cppArg_to_pyArg(data, CXX_Struct, selfType->u, selfType->parentModule);
    }
    (selfType->_i)++;
...
static PyObject *c_union_append(PyObject *self, PyObject *args) {
    ...
    if (!(selfType->isArray)) {
        PyErr_SetString(py_CppError, "given instance of c_union is not an array type instance");
        return NULL;
    }
    ...
    if (Py_TYPE(value) != Py_TYPE(self)) {
        PyErr_SetString(py_BindingError, "Expected arg to be of the same union type");
        return NULL;
    }
    ...
    if (selfType->arrayCapacity > selfType->arraySize) {
        memcpy(selfType->pointer + (selfType->u->unionSize * selfType->arraySize), valueType->pointer,
               selfType->u->unionSize);
    } else {
        int new_capacity        = selfType->arrayCapacity ? (selfType->arrayCapacity * 2) : 2;
        selfType->pointer       = realloc(selfType->pointer, new_capacity * selfType->u->unionSize);
        selfType->arrayCapacity = new_capacity;
    ...
    void *data = selfType->pointer + (selfType->u->unionSize * selfType->arraySize);
...
static PyObject *c_union_pop(PyObject *self) {
    ...
    memcpy(data, ((char *)selfType->pointer) + (selfType->u->unionSize * (selfType->arraySize - 1)),
           selfType->u->unionSize);

    PyObject *tmp = cppArg_to_pyArg(data, CXX_Struct, selfType->u, selfType->parentModule);

    (selfType->arraySize)--;

    if ((selfType->arraySize * 2) < selfType->arrayCapacity) {
        selfType->pointer       = realloc(selfType->pointer, (selfType->arraySize) * selfType->u->unionSize);
...
static int c_union_setitem(PyObject *self, PyObject *attr, PyObject *value) {
    ...
    if (selfType->arraySize > index) {
        ...
        memcpy(selfType->pointer + (selfType->u->unionSize * index), valueType->pointer, selfType->u->unionSize);
...
static int c_union_freeOnDel_setter(PyObject *self, PyObject *value, void *closure) {
    PyC_c_union *selfType = (PyC_c_union *)self;

    if (!value) {
        PyErr_SetString(PyExc_AttributeError, "Cannot delete c_union.free_on_no_reference attrubute");
        return -1;
    }

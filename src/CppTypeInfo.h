...
struct _RecordType {
    ...
    p_void_array_t *attrTypeInfo;    // to store pointee struct & union, length of array
    size_t attrCount;
};
...
struct _FunctionType {
    ...
    size_t argsCount;
    p_void_array_t *argsTypeInfo; // to store pointee struct or union, length of array
...
bool Symbols_appendTypedef(Symbols *sym, char *typedefName, char *typeName, enum CXX_Type type, void *extra_info);
